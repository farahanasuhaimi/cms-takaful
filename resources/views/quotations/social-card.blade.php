    ...
    <div class="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div class="flex-1 flex justify-center overflow-x-auto py-4">
            <div id="social-card" class="relative flex-shrink-0 overflow-hidden rounded-[28px]"
                ...
                {{-- Contact bar --}}
                <div class="absolute top-4 left-4 right-4 bg-white/90 rounded-full shadow-sm px-4 py-2 flex items-center gap-3 text-[11px] font-medium text-gray-700 z-10">
                    <span class="flex items-center gap-1">
                        <svg class="w-3 h-3 text-strawberry-500" fill="currentColor" viewBox="0 0 20 20"><path d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z"/></svg>
                        {{ $card['name'] }}
                    </span>
                    @if ($card['phone'])
                        <span class="text-gray-300">|</span>
                        <span class="flex items-center gap-1">
                            <svg class="w-3 h-3 text-matcha-600" fill="currentColor" viewBox="0 0 20 20"><path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z"/></svg>
                            {{ $card['phone'] }}
                        </span>
                    @endif
                    @if ($card['website'])
                ...
                {{-- Title --}}
                <div class="absolute top-16 left-6 right-6 z-10">
                    <p class="text-[26px] font-extrabold leading-tight text-matcha-900">{{ $quotation->title }}</p>
                    @if ($plan)
                        <p class="text-strawberry-600 font-bold text-sm mt-0.5">{{ $plan->plan_name }}</p>
                    @endif
                </div>

                {{-- Comparison table --}}
                <div class="absolute top-36 left-6 w-[230px] z-10">
                    <div class="rounded-xl overflow-hidden shadow-sm border border-white/60">
                ...
                {{-- Highlights panel --}}
                <div class="absolute top-36 right-6 w-[230px] z-10 space-y-2">
                    @foreach (array_slice($highlights, 0, 4) as $h)
                        <div class="flex items-start gap-1.5 bg-white/70 rounded-lg px-2 py-1.5">
                            ...
                            <span class="text-[10px] leading-snug text-gray-700">{{ $h }}</span>
                        </div>
                    @endforeach
                </div>

                {{-- Decorative leaf motif filling the middle whitespace --}}
                <svg class="absolute top-[230px] left-8 w-24 h-24 text-matcha-200 opacity-70 z-0" viewBox="0 0 100 100" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <svg class="absolute top-[300px] left-[150px] w-6 h-6 text-strawberry-300 opacity-80 z-0" fill="currentColor" viewBox="0 0 20 20">
                ...
                @if ($card['phone'] || $card['website'])
                    <div class="absolute bottom-24 right-6 z-10 transform rotate-6">
                        <div class="bg-amber-400 border-2 border-dashed border-amber-600 rounded-xl px-4 py-2.5 shadow-md text-center">
                ...
                {{-- Bottom CTA banner --}}
                @if ($card['cta'])
                    <div class="absolute bottom-4 left-4 right-4 bg-matcha-900 rounded-xl px-4 py-3 z-10">
                        <p class="text-white text-[11px] leading-snug font-medium">{{ $card['cta'] }}</p>
                    </div>
                @endif
